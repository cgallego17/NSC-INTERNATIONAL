...
for country in data:
    ...
    if (
        ...
        panama_country = country
        break

if not panama_country:
    print("❌ No se encontró Panamá")
    exit(1)

print(
    f"\n✓ País encontrado: {panama_country['name']} (ISO2: {panama_country.get('iso2', 'N/A')})"
)

states_data = panama_country.get("states", [])
total_cities = sum(len(state.get("cities", [])) for state in states_data)

print(f"  - Provincias: {len(states_data)}")
...
try:
    with transaction.atomic():
        ...
        country, created = Country.objects.get_or_create(
            code=panama_country.get("iso2", "PA").upper(),
            defaults={
                "name": "Panamá",
                "is_active": True,
            },
        ...
        for idx, state_data in enumerate(states_data, 1):
            ...
            # Importar ciudades de la provincia
            ...
            # Mostrar progreso cada 10 provincias
            if idx % 10 == 0:
                print(f"  Procesados {idx}/{len(states_data)} provincias...")
                sys.stdout.flush()

        print("\n✅ Importación completada!")
        print(f"   - País: {'Creado' if created else 'Ya existía'}")
        print(f"   - Provincias creadas: {states_created}")
        ...
        print(f"\n📊 Verificación:")
        print(f"   - Provincias en BD: {final_states}")
