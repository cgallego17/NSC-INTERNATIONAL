...
print("=" * 60)
print("IMPORTACIÓN DE VENEZUELA")
print("=" * 60)
sys.stdout.flush()
...
for country in data:
    ...
    if (
        ...
        venezuela_country = country
        break

if not venezuela_country:
    print("❌ No se encontró Venezuela")
    exit(1)

print(
    f"\n✓ País encontrado: {venezuela_country['name']} (ISO2: {venezuela_country.get('iso2', 'N/A')})"
)

states_data = venezuela_country.get("states", [])
...
try:
    with transaction.atomic():
        ...
        country, created = Country.objects.get_or_create(
            code=venezuela_country.get("iso2", "VE").upper(),
            defaults={
                "name": "Venezuela",
                "is_active": True,
            },
