...
from .models import EventCategory, Event, EventAttendance, EventComment, EventReminder
...
class EventAdmin(admin.ModelAdmin):
    list_display = ['title', 'category', 'start_date', 'end_date', 'status', 'priority', 'organizer', 'attendees_count', 'is_public']
    ...
    fieldsets = (
        ('Información Básica', {
            'fields': ('title', 'description', 'short_description', 'category', 'image')
        }),
        ('Estado y Prioridad', {
        ...
        ('Organización', {
            'fields': ('organizer', 'is_public', 'requires_registration', 'max_attendees')
        }),
        ('Enlaces y Notas', {
            'fields': ('external_link', 'notes')
        }),
        ('Estadísticas', {
            'fields': ('attendees_count', 'is_past', 'is_ongoing', 'is_upcoming', 'is_full'),
            'classes': ('collapse',)
        }),
        ('Fechas del Sistema', {
