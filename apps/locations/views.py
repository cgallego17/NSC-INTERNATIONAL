...
from .models import Country, State, City, Season, Rule, Site
...
class SiteCreateView(LoginRequiredMixin, CreateView):
    ...
    fields = [
        'site_name', 'abbreviation', 'address_1', 'address_2', 
        'city', 'state', 'postal_code', 'country', 'website', 
        'image', 'additional_info', 'is_active'
    ]
...
def sites_api(request):
    ...
    sites = Site.objects.filter(is_active=True).select_related('state', 'city', 'country').values(
        'id', 
        'site_name', 
        'abbreviation',
        'state_id',
