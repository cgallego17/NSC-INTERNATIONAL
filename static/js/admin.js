...
document.addEventListener('DOMContentLoaded', () => {
    // Ensure modern toast styles exist (some pages include admin.js but not admin.css/panel_usuario.css)
    // Without these styles, registration toasts can appear "without CSS".
    (function ensureModernToastStyles() {
        try {
            // Avoid duplicating the injected stylesheet
            if (document.getElementById('nsc-modern-toast-styles')) return;
            if (!document.body) return;

            // Probe if .modern-toast is styled by any loaded CSS
            const probe = document.createElement('div');
            probe.className = 'modern-toast';
            ...
            const borderRadius = cs ? cs.borderRadius : '';
            const boxShadow = cs ? cs.boxShadow : '';
            document.body.removeChild(probe);

            // If styles are missing, inject a minimal CSS set for modern toasts
            const looksUnstyled =
                (!borderRadius || borderRadius === '0px' || borderRadius === '0') &&
                (!boxShadow || boxShadow === 'none');

            if (!looksUnstyled) return;

            const style = document.createElement('style');
            style.id = 'nsc-modern-toast-styles';
            style.textContent = `
                .modern-toast-container{position:fixed;top:20px;right:20px;z-index:99999;display:flex;flex-direction:column;gap:12px;max-width:400px;pointer-events:none}
                .modern-toast{position:relative;background:#fff;border-radius:12px;box-shadow:0 10px 40px rgba(13,44,84,.15),0 4px 12px rgba(0,0,0,.1);overflow:hidden;opacity:0;transform:translateX(400px) scale(.95);transition:all .3s cubic-bezier(.68,-.55,.265,1.55);pointer-events:all;min-width:320px;max-width:400px;border:1px solid rgba(13,44,84,.1)}
                .modern-toast.show{opacity:1;transform:translateX(0) scale(1)}
                .modern-toast.hide{opacity:0;transform:translateX(400px) scale(.95)}
                .toast-content{display:flex;align-items:flex-start;padding:16px 20px;gap:14px;position:relative;z-index:2}
                .toast-icon{width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:18px;color:#fff;box-shadow:0 4px 12px rgba(0,0,0,.15)}
                .toast-body-wrapper{flex:1;min-width:0}
                .toast-title{font-weight:700;font-size:16px;margin-bottom:4px;color:#0d2c54;line-height:1.4;font-family:Arial,Helvetica,sans-serif}
                .toast-message{font-size:14px;color:#2c3e50;line-height:1.6;word-wrap:break-word;font-family:Arial,Helvetica,sans-serif}
                .toast-close{background:none;border:none;color:#a0aec0;cursor:pointer;padding:4px;width:24px;height:24px;display:flex;align-items:center;justify-content:center;border-radius:6px;transition:all .2s ease;flex-shrink:0;font-size:12px}
                .toast-close:hover{background:rgba(0,0,0,.05);color:#2d3748;transform:rotate(90deg)}
                .toast-progress{position:absolute;bottom:0;left:0;right:0;height:3px;background:rgba(0,0,0,.05);overflow:hidden}
                .toast-progress-bar{height:100%;width:100%;transform-origin:left;animation:progressShrink linear forwards;animation-play-state:paused}
                @keyframes progressShrink{from{transform:scaleX(1)}to{transform:scaleX(0)}}
                .toast-success{border-left:4px solid #0d2c54;background:linear-gradient(135deg,#fff 0%,#f8faff 100%)}
                .toast-success .toast-icon{background:linear-gradient(135deg,#0d2c54 0%,#132448 100%)}
                .toast-success .toast-progress-bar{background:linear-gradient(90deg,#0d2c54 0%,#132448 100%)}
                .toast-error{border-left:4px solid #d50032;background:linear-gradient(135deg,#fff 0%,#fff8f8 100%)}
                .toast-error .toast-icon{background:linear-gradient(135deg,#d50032 0%,#b50028 100%)}
                .toast-error .toast-progress-bar{background:linear-gradient(90deg,#d50032 0%,#b50028 100%)}
                .toast-warning{border-left:4px solid #ffc107;background:linear-gradient(135deg,#fff 0%,#fffef8 100%)}
                .toast-warning .toast-icon{background:linear-gradient(135deg,#ffc107 0%,#e0a800 100%)}
                .toast-warning .toast-progress-bar{background:linear-gradient(90deg,#ffc107 0%,#e0a800 100%)}
                .toast-info{border-left:4px solid #0d2c54;background:linear-gradient(135deg,#fff 0%,#f8f9fc 100%)}
                .toast-info .toast-icon{background:linear-gradient(135deg,#0d2c54 0%,#132448 100%)}
                .toast-info .toast-progress-bar{background:linear-gradient(90deg,#0d2c54 0%,#132448 100%)}
                .toast-primary{border-left:4px solid #0d2c54;background:linear-gradient(135deg,#fff 0%,#f8f9fc 100%)}
                .toast-primary .toast-icon{background:linear-gradient(135deg,#0d2c54 0%,#132448 100%)}
                .toast-primary .toast-progress-bar{background:linear-gradient(90deg,#0d2c54 0%,#132448 100%)}
                @media (max-width:768px){
                    .modern-toast-container{top:10px;right:10px;left:10px;max-width:none}
                    .modern-toast{min-width:auto;max-width:none;transform:translateY(-100px) scale(.95)}
                    .modern-toast.show{transform:translateY(0) scale(1)}
                    .modern-toast.hide{transform:translateY(-100px) scale(.95)}
                }
            `;
